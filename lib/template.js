...
export function buildEmail({ name, replyTo, personalizedHook, personalizedSubject }) {
  const cleanName = cleanCompanyName(name);
  const subject =
    (personalizedSubject && personalizedSubject.trim()) ||
    `monitoramento de uptime pra ${cleanName}`;
