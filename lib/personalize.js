...
function formatInsights(insights) {
  if (!insights) return "";
  const stack = insights.techStack?.length
    ? `STACK: ${insights.techStack.join(", ")}`
    : "";
  const pains = insights.painSignals?.length
    ? `SINAIS: ${insights.painSignals
        .slice(0, 4)
        .map((s) => `${s.label} (${s.detail})`)
        .join(" | ")}`
    : "";
  const pages = insights.pages?.length
    ? `PÁGINAS: ${insights.pages
        .filter((p) => p.ok)
        .slice(0, 5)
        .map((p) => p.path)
        .join(", ")}`
    : "";
  return [stack, pains, pages].filter(Boolean).join("\n");
}

function buildUserPrompt(name, signals, insights) {
  const techSignals = formatInsights(insights);
  return `Empresa: ${name}
...
Análise técnica:
${techSignals || "(sem análise técnica salva)"}
...
export async function personalizeLead(lead) {
  ...
  const completion = await groq.chat.completions.create({
    ...
    messages: [
      ...
      { role: "user", content: buildUserPrompt(lead.name, signals, lead.siteInsights) },
    ],
  });
...
export async function personalizeLeads(
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    try {
      const { hook } = await personalizeLead(lead);
